...
const LAB_EXPRESSIONS_DIR = 'expressions';
const LAB_ARCHIVE_MANIFEST_PATH = `${LAB_ARCHIVE_DIR}/manifest.json`;
const LAB_WEIGHT_BOUNDS = { min: -3, max: 3 };
...
let LabProjectSaveRootHandle = null;
...
function getTargetSaveDirectorySegments(subdirectoryName) {
    ...
    if (LabProjectSaveRootHandle && LabProjectSaveRootHandle.name === versionDirectory) {
...
async function saveBlobToVersionSubdirectory(blob, fileName, subdirectoryName) {
    if (typeof window !== 'undefined' && typeof window.showDirectoryPicker === 'function') {
        try {
            if (!LabProjectSaveRootHandle) {
                LabProjectSaveRootHandle = await window.showDirectoryPicker({
...
async function randomizeNetwork() {
    ...
    try {
        const genome = NEAT.Genome.createMinimal();
...
async function downloadPreviewImage(resolution = LAB_DOWNLOAD_GIF_RESOLUTION) {
    ...
    try {
        ...
        await saveBlobToVersionSubdirectory(
            blob,
            `activation-lab-output-${genomeId}.gif`,
            LAB_EXPRESSIONS_DIR
        );
    } catch (error) {
...
async function downloadLabGenomeJson() {
    if (!LabState.genome) return;
    ...
    await saveBlobToVersionSubdirectory(
        blob,
        `activation-lab-genome-${genomeId}.json`,
        LAB_ARCHIVE_DIR
    );
}
