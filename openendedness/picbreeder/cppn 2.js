...
class CPPNNetwork {
    ...
    activate(x, y) {
        // PicBreeder-style inputs: x, y, distance, bias(=1)
...
class CPPNRenderer {
    render(genome, canvas, resolution = null) {
        const ctx = canvas.getContext('2d');
        const width = resolution || canvas.width;
        const height = resolution || canvas.height;
        ...
        // PicBreeder normalizes to [-0.5, 0.5] range
        const scale = 1.0;
        ...
        for (let py = 0; py < height; py++) {
            for (let px = 0; px < width; px++) {
                ...
                data[idx] = Math.round(Math.max(0, Math.min(1, rgb.r)) * 255);
                data[idx + 1] = Math.round(Math.max(0, Math.min(1, rgb.g)) * 255);
                data[idx + 2] = Math.round(Math.max(0, Math.min(1, rgb.b)) * 255);
                data[idx + 3] = 255;
            }
        }
    ...
    renderProgressive(genome, canvas, onComplete) {
        this.render(genome, canvas, 64);

        requestAnimationFrame(() => {
            const size = Math.min(canvas.clientWidth, canvas.clientHeight) || 150;
            this.render(genome, canvas, size);
